...
void UPsIronSource_Android::ShowRewardedVideo(const FString& PlacementName, const FString& ParamKey, const FString& ParamValue) const
{
	LOGD("%s", TCHAR_TO_ANSI(*PS_FUNC_LINE));
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv(true))
	{
		UPsIronSource_Android::AndroidThunkJava_IronSource_showRewardedVideo = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_IronSource_showRewardedVideo", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V", false);

		jstring JPlacementName = Env->NewStringUTF(TCHAR_TO_UTF8(*PlacementName));
		jstring JParamKey = Env->NewStringUTF(TCHAR_TO_UTF8(*ParamKey));
		jstring JParamValue = Env->NewStringUTF(TCHAR_TO_UTF8(*ParamValue));
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, UPsIronSource_Android::AndroidThunkJava_IronSource_showRewardedVideo, JPlacementName, JParamKey, JParamValue);
		Env->DeleteLocalRef(JPlacementName);
		Env->DeleteLocalRef(JParamKey);
		Env->DeleteLocalRef(JParamValue);
	}
	else
	{
		LOGD("%s: invalid JNIEnv", TCHAR_TO_ANSI(*PS_FUNC_LINE));
	...
	if (ISProxy != nullptr)
	{
		...
		AsyncTask(ENamedThreads::GameThread, []() {
			if (ISProxy != nullptr)
			{
				...
				ISProxy->VideoStateDelegate.Broadcast(2);
			}
			else
			{
...
JNI_METHOD void Java_com_epicgames_ue4_GameActivity_onRewardedVideoAdClosedThunkCpp(JNIEnv* jenv, jobject thiz)
{
	if (ISProxy != nullptr)
	{
		...
		AsyncTask(ENamedThreads::GameThread, []() {
			...
			{
				ISProxy->DequeueEvent();
				ISProxy->VideoStateDelegate.Broadcast(3);
			}
			else
			{
				LOGD("%s: invalid ISProxy", TCHAR_TO_ANSI(*PS_FUNC_LINE));
...
JNI_METHOD void Java_com_epicgames_ue4_GameActivity_onRewardedVideoAvailabilityChangedThunkCpp(JNIEnv* jenv, jobject thiz, jboolean available)
{
	...
	AsyncTask(ENamedThreads::GameThread, [available]() {
		if (ISProxy != nullptr)
		{
			if (available)
			{
				ISProxy->VideoStateDelegate.Broadcast(8);
			}
			else
			{
				ISProxy->VideoStateDelegate.Broadcast(7);
			}
		}
		else
		{
...
JNI_METHOD void Java_com_epicgames_ue4_GameActivity_onRewardedVideoAdStartedThunkCpp(JNIEnv* jenv, jobject thiz)
{
	if (ISProxy != nullptr)
	{
		...
		AsyncTask(ENamedThreads::GameThread, []() {
			...
			{
				ISProxy->DequeueEvent();
				ISProxy->VideoStateDelegate.Broadcast(4);
			}
			else
			{
				LOGD("%s: invalid ISProxy", TCHAR_TO_ANSI(*PS_FUNC_LINE));
...
JNI_METHOD void Java_com_epicgames_ue4_GameActivity_onRewardedVideoAdEndedThunkCpp(JNIEnv* jenv, jobject thiz)
{
	if (ISProxy != nullptr)
	{
		...
		AsyncTask(ENamedThreads::GameThread, []() {
			...
			{
				ISProxy->DequeueEvent();
				ISProxy->VideoStateDelegate.Broadcast(5);
			}
			else
			{
				LOGD("%s: invalid ISProxy", TCHAR_TO_ANSI(*PS_FUNC_LINE));
...
JNI_METHOD void Java_com_epicgames_ue4_GameActivity_onRewardedVideoAdRewardedThunkCpp(JNIEnv* jenv, jobject thiz)
{
	if (ISProxy != nullptr)
	{
		...
		AsyncTask(ENamedThreads::GameThread, []() {
			...
			{
				ISProxy->DequeueEvent();
				ISProxy->VideoStateDelegate.Broadcast(0);
			}
			else
			{
				LOGD("%s: invalid ISProxy", TCHAR_TO_ANSI(*PS_FUNC_LINE));
...
JNI_METHOD void Java_com_epicgames_ue4_GameActivity_onRewardedVideoAdShowFailedThunkCpp(JNIEnv* jenv, jobject thiz, jint errorCode, jstring errorMessage)
{
	if (ISProxy != nullptr)
	{
		...
		if (errorMessage != nullptr)
		{
			...
			const char* UTFString = Env->GetStringUTFChars(errorMessage, nullptr);
			Result = FString(UTF8_TO_TCHAR(UTFString));
			Env->ReleaseStringUTFChars(errorMessage, UTFString);
			Env->DeleteLocalRef(errorMessage);
		}
		
		AsyncTask(ENamedThreads::GameThread, [Result, errorCode]() {
			if (ISProxy != nullptr)
			{
				ISProxy->DequeueEvent();
				ISProxy->VideoStateDelegate.Broadcast(1);
				UE_LOG(LogTemp, Error, TEXT("onRewardedVideoAdShowFailed %d, %s"), (int32)errorCode, *Result);
			}
			else
			{
				LOGD("%s: invalid ISProxy", TCHAR_TO_ANSI(*PS_FUNC_LINE));
...
JNI_METHOD void Java_com_epicgames_ue4_GameActivity_onRewardedVideoAdClickedThunkCpp(JNIEnv* jenv, jobject thiz)
{
	if (ISProxy != nullptr)
	{
		...
		AsyncTask(ENamedThreads::GameThread, []() {
			...
			{
				ISProxy->DequeueEvent();
				ISProxy->VideoStateDelegate.Broadcast(6);
			}
			else
			{
				LOGD("%s: invalid ISProxy", TCHAR_TO_ANSI(*PS_FUNC_LINE));
	...
	if (ISProxy != nullptr)
	{
		...
		AsyncTask(ENamedThreads::GameThread, [Data]() {
			if (ISProxy != nullptr)
			{
				...
				ISProxy->VideoStateDelegate.Broadcast(9);
			}
			else
			{
				LOGD("%s: invalid ISProxy", TCHAR_TO_ANSI(*PS_FUNC_LINE));
